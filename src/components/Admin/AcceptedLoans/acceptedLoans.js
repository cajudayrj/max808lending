...
const AcceptedLoans = () => {
  ...
  useEffect(() => {
    ...
    axios(`${serverUrl}/loans/accepted/page/${currentPage}`, {
      method: 'GET',
      headers: {
      ...
      .then(({ data }) => {
        const { acceptedLoans, totalPage } = data;
        setAcceptedLoans(acceptedLoans);
  ...
  return (
    <div className="admin-loans-dashboard accepted-loan-dashboard">
      <h1 className="header-title">accepted Loans</h1>
          ...
          <tbody>
            {
              acceptedLoans.length > 0 ?
                ...
                :
                <tr>
                  <td className="no-loans" colSpan={7}>No accepted loans found.</td>
                </tr>
            }
          </tbody>
        </table>
